...
# Exotel's plan caps a streaming session around 60 minutes; wrap up before that.
...
async def bot(runner_args: RunnerArguments):
    ...
    tts = SarvamTTSService(
        ...
        # downsampled to Twilio's/Exotel's required 8kHz on every streamed
        # chunk — an extra resampling stage on small, frequent chunks is a
        # known source of choppy/robotic-sounding audio. Generating natively
        # at 8000 (a Sarvam-documented supported rate) skips that step.
        sample_rate=8000,
    ...
    async def on_client_connected(transport, client):
        ...
        # so composing it live cost 1.5-2.5s of LLM+aggregation latency per
        # call at the moment callers are most impatient — and earlier it also
        # let the LLM pick Hindi on its own. Deterministic text kills both.
        # TTSSpeakFrame output lands in the LLM context as an assistant
        # message (confirmed in real call logs — the idle nudges show up
        # there), so the LLM knows it already greeted.
